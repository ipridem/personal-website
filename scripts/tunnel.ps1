# Starts a local static server + a Cloudflare quick tunnel, prints the public
...
#
# Usage (this project — serves the ipridem portfolio):
...
param(
    [string]$Command = "python -m http.server 8000",  # command that starts your server
    [int]$Port = 8000,                                # port that command listens on
...
Write-Host "Starting server: $Command" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "Stopping tunnel and server..." -ForegroundColor Cyan
    # servers that fork a child (reloaders/watchers) outlive Stop-Process on the
    # parent alone; /T kills the whole tree.
